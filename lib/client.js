...
class EventEmitter {
  ...
  off(event, handler = null) {
    if (this.events[event] == null) { this.events[event] = []; }
    this.events[event] = this.events[event].filter(fn => (handler !== null) && (fn !== handler));
  }
}
...
DocumentRef = class DocumentRef extends EventEmitter {
  ...
  on(event, handler) {
    ...
    if (
      ((this.events.update != null ? this.events.update.length : undefined) > 0)
      || ((this.events.value != null ? this.events.value.length : undefined) > 0)
    ) {
  ...
  set(value, next) {
    ...
    if (this.database.safe_writes) {
      ...
      if (!allow) { return (typeof next === 'function' ? next('cannot set a field that was not queried for') : undefined); }
    }
    ...
    return ref.set(value, (err) => {
      ...
      return this.database.request(`sync/${this.key}`, (syncErr) => {
        ...
        return this.updateData(value, () => (typeof next === 'function' ? next(null) : undefined));
      });
    });
  }
